function x = blockMGST(x,b,l,Ax,LXI)
...
if l > lmin
    ...
    x = blockSmoothMGST(x,b,l,nu2,Ax,LXI);
...
%     A   = eye(nl) + C_glob{l} + D_glob{l};
    ...
    % To add spatial component to res or not? It's not part of the
    % multigrid scheme so no? Seems to converge faster without including it
  ...
  %  norm(res,2)/sqrt(Nx*n)
    ...
    if direct_solve
%        nl  = 2^(l+1);
        x = blockDirectSolve(x,b,l,Ax,LXI);
%         for ii = 2:Nx 
%             duu = x(ii,:)';
...
%         end
%         for k = 1:nl
%              x(1:Nx,k) = LXI*x(1:Nx,k); 
%         end
    else
        x = blockSmoothMGST(x,b,l,nu1,Ax,LXI);
    end
end
